DO $$ BEGIN
 CREATE TYPE "user_account_role" AS ENUM('GLOBAL_ADMIN', 'USER');
EXCEPTION
...
CREATE TABLE IF NOT EXISTS "global_application_settings" (
...
CREATE TABLE IF NOT EXISTS "user_account_tenant" (
...
DO $$ BEGIN
 ALTER TABLE "user_account_tenant" ADD CONSTRAINT "user_account_tenant_fk_tenant_id_tenant_tenant_id_fk" FOREIGN KEY ("fk_tenant_id") REFERENCES "tenant"("tenant_id") ON DELETE no action ON UPDATE no action;
EXCEPTION
 WHEN duplicate_object THEN null;
END $$;

DO $$ BEGIN
 ALTER TABLE "user_account_tenant" ADD CONSTRAINT "user_account_tenant_fk_user_account_id_user_account_user_account_id_fk" FOREIGN KEY ("fk_user_account_id") REFERENCES "user_account"("user_account_id") ON DELETE no action ON UPDATE no action;
EXCEPTION
 WHEN duplicate_object THEN null;
END $$;
...
CREATE UNIQUE INDEX IF NOT EXISTS "tenant_user_account_uq_idx" ON "user_account_tenant" ("fk_tenant_id","fk_user_account_id");
